...
describe('RoleService', () => {
  ...
  describe('update', () => {
   ...
   it('should throw BadRequestException if the role name already exists', async () => {
    ...
    await expect(service.update('existingRoleId', updateDto)).rejects.toThrow(BadRequestException);
  ...
  it('should update the role if it exists and name is not taken', async () => {
    ...
    jest.spyOn(model, 'findById').mockResolvedValue(mockRole as Role);
    jest.spyOn(model, 'findOne').mockResolvedValue(null);
    jest.spyOn(model, 'findByIdAndUpdate').mockResolvedValue(mockUpdatedRole as Role);
    ...
    expect(model.findOne).toHaveBeenCalledWith({ name: updateDto.name });
    expect(model.findByIdAndUpdate).toHaveBeenCalledWith('roleId', updateDto, { new: true });
  });
  ...
  describe('delete', () => {
    ...
    it('should delete the role if it exists', async () => {
      const mockRole = { _id: 'roleId', name: 'ExistingRole', deleteOne: jest.fn() };
