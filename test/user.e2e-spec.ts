...
describe('User (e2e)', () => {
  ...
  beforeEach(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [CustomerModule, CompanyInstallerModule, SeedModule, AppModule],
    }).compile();
  ...
  describe('customerController', () => {
    describe('/customer (POST)', () => {
      ...
      it('should return 400 if the user sends the same email', async () => {
        ...
        await request(app.getHttpServer()).post(PATH).send(newUser).expect(400);
      });
    ...
    describe('/customer (PUT)', () => {
      ...
      it('should return 400 if the user dont send a token', async () => {
        await request(app.getHttpServer()).put(`${PATH}`).expect(400);
      });
    });
  ...
  describe('installerController', () => {
    ...
    describe('/installer/status/:id (PUT)', () => {
      ...
      it('should return 400 if the user sends an invalid id', async () => {
        await request(app.getHttpServer())
          .put(`${PATH_INSTALLER}/status/12345`)
          .send({ status: 'approved' })
          .expect(400);
      });
      ...
      it('should return 404 if the user does not send anything, valid id and not exists', async () => {
        await request(app.getHttpServer())
          .put(`${PATH_INSTALLER}/status/${new mongoose.Types.ObjectId()}`)
          .send({ status: 'approved' })
          .expect(404);
      });
    });
    ...
    describe('/installer/:id (GET)', () => {
      ...
      it('should return 404 if the user does not send anything, valid id and not exists', async () => {
        await request(app.getHttpServer())
          .get(`${PATH_INSTALLER}/${new mongoose.Types.ObjectId()}`)
          .expect(404);
      });

      it('should return 400 if the user sends an invalid id', async () => {
        await request(app.getHttpServer())
          .get(`${PATH_INSTALLER}/12345`)
          .expect(400);
      });
    });
